    ...
    size_t filter_stride)
{
    for (int i = 0; i < AVX512_BS; i++) {
        ...
        // Register pressure: FC=4,OC=6 was 24 acc+6 in_bc+1 filter=31 ZMMs,
...
static SCONV_FORCEINLINE void ComputePointwiseBlock(
    ...
    size_t filter_stride)
{
    // f-outer: filter loaded once per (f,i), reused for all OC.
    // in_bc[] eliminated — broadcast inlined into _mm512_fmadd_ps.
    // Compiler emits: vfmadd231ps zmm_acc, zmm_fvec, [mem]{1to16}
    for (int i = 0; i < AVX512_BS; i++) {
        for (int f = 0; f < FC; f++) {
            const float* fp = reinterpret_cast<const float*>(
