...
static void Avx512PostProcess(
    ...
    unsigned flags)
{
    // Single pass: accumulate + bias + relu + store merged into one for-f for-o loop.
    // Eliminates 3 extra loop nests and avoids re-traversing the FC×OC accumulator array.
    ...
    for (int f = 0; f < FC; f++) {
        ...
        for (int o = 0; o < OC; o++) {
            __m512 v = acc[f][o];
            if (do_accum) v = _mm512_add_ps(v, _mm512_loadu_ps(row + o * AVX512_BS));  // [LOAD-OUTPUT]
            if (do_bias)  v = _mm512_add_ps(v, bvec);
